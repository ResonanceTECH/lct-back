...
export class RedisService {
	...
	setHash(key: string, data: Record<string, string | number | boolean | Date>): Promise<string> {
	...
	deleteValue(...keys: string[]): Promise<number> {
		return this.client.del(keys);
	}

	exists(key: string): Promise<boolean> {
		return this.client.exists(key).then(Boolean);
	}
}
